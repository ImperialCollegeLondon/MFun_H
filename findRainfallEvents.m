function [radarIsEvent] = findRainfallEvents( radarWAR , thresholdWAR , sepHour, thresholdTime, radarReso)
% -------------------------------------------------------------------------
% seperateRainfallEvents Define wet event base on the climate model data
%
% Inputs:
...
%
% Example: 
%    [lengthi,stormsi] = findRainfallEvents( WAR, 0.02, 2, 60);
%    WAR(WAR < 0.02) = 0;
%    
%    hold on
...
for stormi = 1:length(stormsi)
    starti = stormsi(stormi);
    eventlength = lengthi(stormi);
    radarIsEvent(starti:(starti+eventlength-1)) = stormi;
end
