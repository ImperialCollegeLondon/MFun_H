...
p.addParameter('y_label', 'Flow (m^3/s)', @ischar);
p.addParameter('x_label', 'Percentage of time flow exceeded', @ischar);
...
p.addParameter('SeasonPartition', [], @iscell); %If used, cell needs to contain a serial date vector and a vector of months; e.g. {sd, [1 2 3]}
p.addParameter('MonthPartition', [], @isnumeric); %If used, this param requires a serial date vector
...
if ~isempty(p.Results.MonthPartition) && ((size(y_data,2) > 1) || (~isempty(p.Results.SeasonPartition)))
...
if p.Results.ProbabilityOnYAxis
    semilogx(sort(y_data, p.Results.SortDirection), tmp_x, p.Results.Linetype,'LineWidth',p.Results.LineWidth);
else
    semilogx(tmp_x', sort(y_data, p.Results.SortDirection), p.Results.Linetype,'LineWidth',p.Results.LineWidth);
end
hold on;
...
if isempty(p.Results.SeasonPartition) && isempty(p.Results.MonthPartition)
    ret = [tmp_x', (sort(y_data, p.Results.SortDirection))];
    % ret = [(0:100)' prctile(y_data, (0:100)')]; %Output summary percentiles
    % ret.percentiles_summary = [[{'Percentile'} legend_str]; num2cell(ret.percentiles)];
end

%% Summer & Winter Period
if ~isempty(p.Results.SeasonPartition)
    show_legend = true; %Always show legend if this is specified
    sd = p.Results.SeasonPartition{1};
    [tmp_y, tmp_m, tmp_d] = datevec(sd); %#ok<ASGLU>
    seasonlineType = {':','-.','o:','x'};
    
    for season = 1:length(p.Results.SeasonPartition{2})
        % Seasonal Period
        season_month_nums = p.Results.SeasonPartition{2}{season}; %Specify the Summer months as a vector of month numbers, e.g. [1 2 3] for Jan-Mar
        idx_season = ismember(tmp_m, season_month_nums);
        tmp_x = 0 : 100/(sum(idx_season)-1) : 100;
        loglog(tmp_x, sort(y_data(idx_season,:), 1, p.Results.SortDirection), seasonlineType{season}, 'LineWidth',1);% p.Results.LineWidth);
    end
    ...
    % Outputs
    ret = [(0:100)' prctile(y_data, (0:100)') prctile(y_data(idx_season,:), (0:100)') prctile(y_data(idx_winter,:), (0:100)')]; %Output summary percentiles
    % ret.percentiles_summary = [[{'Percentile'} legend_str]; num2cell(ret.percentiles)];
end
...
if ~isempty(p.Results.MonthPartition)
    ...
    for i = 1:12
        ...
        semilogx(tmp_x, sort(y_data(idx,:), 1, p.Results.SortDirection), '-', 'color',p.Results.TraceColours(trace_colour_num,:), ...
            'LineStyle',month_plot_line_styles{line_type_num}, 'LineWidth',0.5);
    ...
    % Outputs
    ret = [(0:100)' prctile(y_data, (0:100)') monthly_percentiles]; %Output summary percentiles
    % ret.percentiles_summary = [[{'Percentile'} legend_str]; num2cell(ret.percentiles)];
end

%% Add Legend
if show_legend
    legend(legend_str, 'FontSize',8, 'Location', 'Southwest');
end
