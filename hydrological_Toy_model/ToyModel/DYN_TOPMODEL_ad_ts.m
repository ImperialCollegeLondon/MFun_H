function [OUTPUT] = DYN_TOPMODEL_ad_ts(TOPO_DAT,HYDR_DATA,METEO_DAT,SIM_PARAM,OUT_PARAM)
...
persistent i; 
if size(ETPt) ~= 1
    fprintf('PET is fully distributed\n');
end
for i = 2:Ndt
    ...
    % Matrix Flow
    Pr = Prt(i)*MASK + QsurR; % [mm/h]  Precipitation<Matrix>
    % Pr = squeeze(Prt(:,:,i)).MASK + QsurR; % [mm/h]  Precipitation<Matrix>
    if size(ETPt) == 1
        ET = squeeze(ETPt(i)).*MASK; % [mm/h]  evapotranspiration<doule>
        if i == 2
            fprinf('PET is spatially uniform.');
        end
    else
        ET = squeeze(ETPt(:,:,i)).*MASK; % [mm/h]  evapotranspiration<Matrix>
    end
    %BETA = ones(m,n);
    ...
    if OPT_UNSAT == 1
        To = (aR.*Ko./f).*(1 -exp(-f.*Zs)); % Trasmsissivity [mm^2/h]  %% 446303.33mm^2/h
        To(isnan(To)) = 0;
    elseif OPT_UNSAT == 2
        Z = V./(Osat-Ohy);% [mm]
        Z(isnan(Z)) = 0;
    ...
    % Leakage rate: https://www.tandfonline.com/doi/pdf/10.1080/02626669209492623
    % 5.67*10^(-8)m/s-->0.2mm/h --> k_sz = 4
    % if OPT_UNSAT == 1 was used:
    %     K_sb = 0.0108mm/h
    %     Ks_Z = 0.0116mm/h
    if OPT_UNSAT == 1
    ...
    if any(Qi(:) > 1) | any(Lk(:) > 1) | any(ET(:) > 1) 
        disp('111');
    end
    
    %V = V -Qi*dth - ET*dth; % Volume second update [mm]%%%%%%%%%%%%%%%%%%%%%%%%%%%% by Yuting
    ...
    % Acquifer Routing
    Qsub= Vf(i)/kF; % [mm/h] % simplified nonliner storage-discharge relationship
    Vf(i)=Vf(i)- Qsub*dth; %[mm]
    QchR= QchR + SN*Qsub/sum(sum(SN));  % [mm/h]
    
    rechargePart = [rechargePart,QchR]; % By Yuting
...
OUTPUT.rechargePart = rechargePart;
OUTPUT.Vf = Vf;
