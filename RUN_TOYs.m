...
%         Prs_spatial = cat(3,Prs_spatial,RADAR_aux.Prs_radar);
        ...
        elseif simNo>200
            ...
            %  NSfilePath = ['F:\Sim_NS_',CatchName,'_new\'];
            %  fileName = ['Rainfall_sim_',num2str(simNo-200),'_',num2str(year_aux),'.mat'];
            %  load([NSfilePath,fileName],'total_rain_map');
            %  total_rain_map = transpose(full(total_rain_map));
            %  total_rain_map = reshape(total_rain_map,32,46,size(total_rain_map,2));% only work for Startford
            %  NS_aux = imresize3(total_rain_map,[154,230,size(total_rain_map,3)],'nearest'); % only work for Startford
            %  Prs_spatial = cat(3,Prs_spatial,NS_aux);
            %  clear total_rain_map NS_aux
